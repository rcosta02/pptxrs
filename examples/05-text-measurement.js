...
async function main() {
  ...
  // ── A. Basic measurement ────────────────────────────────────────────────────
  {
    const m = pres.measureText('Hello, pptrs!', { font: FONT_NAME, fontSize: 36 });
    console.log('\n── A. Basic measurement ────────────────────────────────');
    console.log(`  text    : "Hello, pptrs!"`);
    console.log(`  fontSize: 36 pt`);
    console.log(`  height  : ${m.height.toFixed(1)} pt  (${pts2in(m.height).toFixed(3)} in)`);
    console.log(`  width   : ${m.width.toFixed(1)} pt  (${pts2in(m.width).toFixed(3)} in)`);
  ...
  {
    const longText = 'This is a longer sentence that will wrap when constrained to a narrow box width.';
    const boxWidthIn = 4;
    ...
    console.log(`  no wrap   : ${mNoWrap.lines} line,  width ${mNoWrap.width.toFixed(0)} pt`);
    console.log(`  wrapped   : ${mWrap.lines} lines, height ${mWrap.height.toFixed(0)} pt`);
  }
  ...
  pres.addSlide(null, slide => {
    ...
    const blocks = [
      ...
      { text: 'A subtitle line goes here.', fontSize: 16, bold: false, color: '666666' },
      { text: 'Body paragraph: This text wraps automatically based on measured height, so the next element always starts exactly below it regardless of how long this content is.', fontSize: 14, bold: false, color: '333333' },
      { text: 'Another heading',            fontSize: 20, bold: true,  color: '4472C4' },
      { text: 'Final line of content.',     fontSize: 14, bold: false, color: '555555' },
    ];
    ...
    blocks.forEach(({ text, fontSize, bold, color }) => {
      ...
      slide.addText(text, {
        x: LEFT, y: currentY, w: BOX_W, h: boxH,
        fontSize, bold, color,
        wrap: true,
      });
    ...
    console.log(`\n── C. Dynamic layout ───────────────────────────────────`);
    console.log(`  Stacked ${blocks.length} blocks. Final Y: ${currentY.toFixed(3)} in`);
  });
  ...
  pres.addSlide(null, slide => {
    ...
    samples.forEach((text, i) => {
      ...
      const m   = pres.measureText(text, { font: FONT_NAME, fontSize: 14, width: BOX_W });
      const boxH = pts2in(m.height) + 0.1;

      // Background rect sized to content
      slide.addShape('rect', {
        x: X - 0.05, y: 0.9 - 0.05, w: BOX_W + 0.1, h: boxH + 0.1,
        fill: { color: 'EEF2FF' },
        line: { color: '4472C4', width: 1 },
      });

      slide.addText(text, {
        x: X, y: 0.9, w: BOX_W, h: boxH,
        fontSize: 14, wrap: true, color: '222222',
      });

      // Show measurement annotation
      slide.addText(`${m.lines} line${m.lines !== 1 ? 's' : ''} · ${pts2in(m.height).toFixed(2)}"`, {
        x: X, y: 0.9 + boxH + 0.05, w: BOX_W, h: 0.3,
  ...
  pres.addSlide(null, slide => {
    ...
    const items = [
      { heading: 'Performance',  body: 'Rust compiles to WebAssembly, running at near-native speed in Node.js with no native dependencies.' },
      { heading: 'Compatibility', body: 'Works in Node.js 16+. The output .pptx opens in PowerPoint, Google Slides, LibreOffice, and Keynote.' },
      { heading: 'API Design',   body: 'Mirrors PptxGenJS so migration is straightforward. All coordinates are in inches.' },
      { heading: 'Text Metrics', body: 'Register any TTF/OTF font and get pixel-accurate height and width before export.' },
      { heading: 'JSON Export',  body: 'Serialize a full presentation to JSON for storage, versioning, or template generation.' },
      { heading: 'Import',       body: 'Open any .pptx file, inspect its elements, modify them, and re-export.' },
    ];
    ...
    items.forEach(({ heading, body }) => {
      const mHead = pres.measureText(heading, { font: FONT_NAME, fontSize: HEAD_SZ, bold: true, width: COL_W });
      const mBody = pres.measureText(body,    { font: FONT_NAME, fontSize: BODY_SZ, width: COL_W });
      ...
      slide.addText(heading, {
        x: X, y: colY, w: COL_W, h: headH,
        fontSize: HEAD_SZ, bold: true, color: '4472C4',
      });

      slide.addText(body, {
        x: X, y: colY + headH, w: COL_W, h: bodyH,
        fontSize: BODY_SZ, color: '444444', wrap: true,
      });
